(async function () {
  ...
  async function getWebApiUrl() {
    try {
      const versionArray = Xrm.Utility.getGlobalContext()
        .getVersion()
        .split(".");
      ...
      const requestUrl =
        apiUrl +
        "EntityDefinitions?$select=EntitySetName&$filter=(LogicalName eq %27" +
        entityLogicalName +
        "%27)";
      ...
      const newLocation =
        window.location.origin + apiUrl + pluralName + "(" + recordId + ")";
      return newLocation;
    } catch (e) {
  ...
  if (window.Xrm && window.Xrm.Page) {
    ...
    /\/api\/data\/v[0-9][0-9]?.[0-9]\//.test(window.location.pathname)
  ) {
