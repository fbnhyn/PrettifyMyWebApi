(async function () {
  ...
  const clipBoardIcon =
    `<svg style='width:16px;position:absolute' viewBox='0 0 24 24'>
  ...
  try {
    apiUrl = /\/api\/data\/v[0-9][0-9]?.[0-9]\//.exec(
      window.location.pathname
    )[0];
  } catch {
    alert(
      "It seems you are not viewing a form or the dataverse odata web api. If you think this is an error, please contact the author of the extension and he will fix it asap."
    );
    return;
  }
  ...
  async function retrieveAllPluralNames() {
    ...
    const requestUrl =
      apiUrl + "EntityDefinitions?$select=EntitySetName,LogicalName";
  ...
  async function retrievePrimaryNameAndKeyAndPluralName(logicalName) {
    ...
    const requestUrl =
      apiUrl +
      "EntityDefinitions?$select=PrimaryNameAttribute,PrimaryIdAttribute,EntitySetName&$filter=(LogicalName eq '" +
      logicalName +
      "')";
  ...
  async function retrieveLogicalNameFromPluralNameAsync(pluralName) {
    const requestUrl =
      apiUrl +
      "EntityDefinitions?$select=LogicalName,PrimaryIdAttribute&$filter=(EntitySetName eq '" +
      pluralName +
      "')";
  ...
  async function retrieveSearchableAttributes(logicalName) {
    ...
    const requestUrl =
      ...
      logicalName +
      "')/Attributes?$filter=IsValidForAdvancedFind/Value eq true";
  ...
  async function retrieveUpdateableAttributes(logicalName) {
    const requestUrl =
      ...
      "EntityDefinitions(LogicalName='" +
      logicalName +
      "')/Attributes?$filter=IsValidForUpdate eq true";
  ...
  async function retrieveBooleanFieldMetadata(logicalName, fieldname) {
    const requestUrl =
      ...
      "EntityDefinitions(LogicalName='" +
      logicalName +
      "')/Attributes/Microsoft.Dynamics.CRM.BooleanAttributeMetadata?$select=LogicalName&$expand=OptionSet,GlobalOptionSet";
  ...
  async function doLookupInputQuery(
    pluralName,
    fieldToFilterOn,
    fieldToDisplay,
    primaryKeyfieldname,
    ...
    const requestUrl =
      apiUrl +
      `${pluralName}?$top=10&$select=${fieldToDisplay},${primaryKeyfieldname}&$filter=contains(${fieldToFilterOn}, '${query}')`;
  ...
  function determineType(value) {
    ...
    if (isNumber(value)) {
      ...
      // null check is explicit here (===) to prevent matching undefined (which renders as string)
      cls = "null";
  ...
  async function generateApiAnchorAsync(logicalName, guid) {
    ...
    const newLocation =
      apiUrl + escapeHtml(pluralName) + "(" + escapeHtml(formattedGuid) + ")";

    return `<a target='_blank' href='${newLocation}'>${escapeHtml(
      formattedGuid
    )}</a>`;
  }
  ...
  async function generatePreviewUrlAnchor(logicalName, guid) {
    ...
    return `<a class='previewLink' data-pluralName='${escapeHtml(
      pluralName
    )}' data-guid='${escapeHtml(
      formattedGuid
  ...
  async function generateEditAnchor(logicalName, guid) {
    ...
    return `
<a class='editLink' data-logicalName='${escapeHtml(
      logicalName
    )}' data-pluralName='${escapeHtml(pluralName)}' data-guid='${escapeHtml(
      formattedGuid
    )}' href='javascript:'>Edit this record</a>     
<div class='editMenuDiv' style='display: none;'>
  ...
  function createSpan(cls, value) {
    return (
      `<span style='display: inline-flex;' class='${escapeHtml(
        cls
      )} field'>${escapeHtml(value)}<span class='copyButton'>` +
      clipBoardIcon +
      `</span></span>`
    );
  }
  ...
  function createSpanForLookup(cls, value) {
    return (
      `<span style='display: inline-flex;' class='${escapeHtml(
        cls
      )} field lookupField'>${escapeHtml(value)}<span class='copyButton'>` +
      clipBoardIcon +
      `</span></span>`
    );
  }
  ...
  function createFieldSpan(cls, value, fieldName) {
    return (
      `<span style='display: inline-flex;' class='${escapeHtml(
        ...
        value
      )}<div class='inputContainer containerNotEnabled' style='display: none;' data-fieldname='${escapeHtml(
        fieldName
      )}'></div><span class='copyButton'>` +
      clipBoardIcon +
      `</span></span>`
    );
  }

  function createLookupEditField(
    displayName,
    guid,
    fieldname,
    lookupTypeValue,
    ...
    fieldname = fieldname
      .substring(1, fieldname.length - 1)
      .substring(0, fieldname.length - 7);

    const formattedGuid = guid?.replace("{", "")?.replace("}", "");
    return `<div class='lookupEditLinks' style='display:none;' data-fieldname='${escapeHtml(
      fieldname
    )}'><span class='link'>   <a href='javascript:' class='searchDifferentRecord lookupEditLink' data-fieldname='${escapeHtml(
      fieldname
    )}'>Edit lookup</a></span><span class='link'>   <a href='javascript:' class='clearLookup lookupEditLink' data-fieldname='${escapeHtml(
      fieldname
    )}'>Clear lookup</a></span><span class='link'>   <a href='javascript:' class='cancelLookupEdit lookupEditLink' data-fieldname='${escapeHtml(
      fieldname
    )}' style='display:none;'>Undo changes</a></span></div><span class='lookupEdit' style='display: none;'><div class='inputContainer containerNotEnabled' data-name='${escapeHtml(
      displayName
    )}' data-id='${escapeHtml(formattedGuid)}' data-fieldname='${escapeHtml(
      fieldname
    )}' data-lookuptype='${escapeHtml(
      lookupTypeValue
    )}' data-navigationproperty='${escapeHtml(
      navigationPropertyValue
    )}'></div></span>`;
  }
  ...
  async function enrichObjectWithHtml(
    jsonObj,
    logicalName,
    pluralName,
    primaryIdAttribute,
    ...
    for (let key in ordered) {
      ...
      if (Array.isArray(value)) {
        ...
        if (Object.values(value).every((v) => typeof v === "object")) {
          for (let nestedKey in value) {
            ...
            ordered[key][nestedKey] = await enrichObjectWithHtml(
              nestedValue,
              null,
              null,
              null,
      ...
      if (typeof value === "object" && value != null) {
        ordered[key] = await enrichObjectWithHtml(
          value,
          null,
          null,
          null,
      ...
      if (keyHasLookupAnnotation(key, ordered)) {
        ...
        const previewUrl = await generatePreviewUrlAnchor(
          lookupTypeValue,
          value
        );
        ...
        lookupFormatted +=
          spaces +
          createSpan(
            determineType(formattedValueValue),
            "Name: " + formattedValueValue
          );
        lookupFormatted += "<br>   ";
        lookupFormatted +=
          spaces +
          createSpan(
            determineType(lookupTypeValue),
            "LogicalName: " + lookupTypeValue
          );
        lookupFormatted += "<br>   ";
        lookupFormatted +=
          spaces +
          createSpan(
            determineType(navigationPropertyValue),
        ...
        lookupFormatted += "</span>";
        lookupFormatted += createLookupEditField(
          formattedValueValue,
          value,
          key,
          lookupTypeValue,
        ...
        ordered[key] =
          createSpanForLookup("null", value) +
          createLookupEditField(null, null, key, null, null);
      } else if (keyHasFormattedValueAnnotation(key, ordered)) {
        ordered[key] = createdFormattedValueSpan(
          cls,
          value,
          key,
          ordered[key + formattedValueType]
        );
        delete ordered[key + formattedValueType];
      } else {
    ...
    if (!isNested) {
      if (
        ...
        newObj["Form Link"] = createLinkSpan(
          "link",
          generateFormUrlAnchor(logicalName, recordId)
        );

        if (isSingleRecord) {
          newObj["Edit this record"] = createLinkSpan(
            "link",
            await generateEditAnchor(logicalName, recordId)
          );
        } else {
          newObj["Web Api Link"] = createLinkSpan(
            "link",
            generateWebApiAnchor(recordId, pluralName)
          );
        }
      } else if (
        logicalName != null &&
  ...
  function setCopyToClipboardHandlers() {
    Array.from(document.querySelectorAll(".copyButton")).forEach(
      (el) =>
        (el.onclick = (element) => {
  ...
  function setPreviewLinkClickHandlers() {
    ...
    for (let previewLink of previewLinks) {
      ...
      const newLocation =
        pluralName + "(" + previewLink.attributes["data-guid"].value + ")";
  ...
  function setImpersonateUserHandlers() {
    const impersonateAnotherUserCheckbox = document.getElementsByClassName(
      "impersonateAnotherUserCheckbox"
    )[0];
    const impersonateAnotherUserSelect = document.getElementsByClassName(
      "impersonateAnotherUserSelect"
    )[0];
    const impersonationIdFieldLabel = document.getElementsByClassName(
      "impersonationIdFieldLabel"
    )[0];
    const impersonateDiv = document.getElementsByClassName("impersonateDiv")[0];
    const impersonateAnotherUserInput = document.getElementsByClassName(
      "impersonateAnotherUserInput"
    )[0];
    const impersonateUserPreview = document.getElementsByClassName(
      "impersonateUserPreview"
    )[0];

    impersonateAnotherUserSelect.onchange = async () => {
      impersonationIdFieldLabel.innerText =
        impersonateAnotherUserSelect.value + ":";
    ...
    async function handleUserPreview() {
      ...
      const retrievedSystemUser = await odataFetch(
        apiUrl +
          `systemusers?$filter=${impersonateAnotherUserSelect.value} eq '${impersonateAnotherUserInput.value}'&$select=fullname`
      );
      if (retrievedSystemUser.error) {
        ...
        impersonateUserPreview.innerText =
          retrievedSystemUser.value[0].fullname;
      } else {
        impersonateUserPreview.innerText =
          "Something went wrong with retrieving the systemuser.";
      }
    }
  }

  function setLookupQueryByIdHandlers(
    input,
    lookupQueryResultPreview,
    selectTable
  ) {
    ...
    async function handlePreview() {
      ...
      const { plural, key, name } =
        await retrievePrimaryNameAndKeyAndPluralName(selectTable.value);

      const retrievedRecord = await odataFetch(
        apiUrl + `${plural}(${input.value})?$select=${name}`
      );
  ...
  function setLookupEditHandlers() {
    ...
    for (let link of clearLookupLinks) {
      ...
      link.onclick = () => {
        const enabledInputFields =
          document.getElementsByClassName("enabledInputField");
        const lookup = [...enabledInputFields].find(
          (f) => f.dataset.fieldname === logicalName
        );
        lookup.value = null;
        ...
        const makeNullDiv = [
          ...document.getElementsByClassName("makeLookupNullDiv"),
        ].find((f) => f.dataset.fieldname === logicalName);
        makeNullDiv.style.display = "block";

        const lookupEditMenuDivs =
          document.getElementsByClassName("lookupEditMenuDiv");
        const lookupEditMenuDiv = [...lookupEditMenuDivs].find(
          (f) => f.dataset.fieldname === logicalName
        );

        lookupEditMenuDiv.style.display = "none";

        const cancelLookupEditDivs =
          document.getElementsByClassName("cancelLookupEdit");
        const cancelLookupEditDiv = [...cancelLookupEditDivs].find(
          (f) => f.dataset.fieldname === logicalName
        );
    ...
    const searchDifferentRecordLinks = document.getElementsByClassName(
      "searchDifferentRecord"
    );
  ...
  function createOptionSetValueInput(container, optionSet) {
    ...
    optionSet.forEach(function (option) {
      ...
      selectHtml += `<option value='${escapeHtml(
        formattedOption
      )}'>${escapeHtml(formattedOption)}</option>`;
    });
  ...
  function createBooleanInput(container, falseOption, trueOption) {
    ...
    const falseFormatted =
      "false : " + falseOption.Label.UserLocalizedLabel.Label;
    const trueFormatted = "true : " + trueOption.Label.UserLocalizedLabel.Label;
    // TODO: refactor the value attribute to contain the pure values, true/false/null
    selectHtml += `<option value='${escapeHtml(falseFormatted)}'>${escapeHtml(
      falseFormatted
    )}</option>`;
    selectHtml += `<option value='${escapeHtml(trueFormatted)}'>${escapeHtml(
      trueFormatted
    )}</option>`;
    select.innerHTML = selectHtml;
  ...
  function createLookupInput(container, targets) {
    ...
    makeNullDiv.innerHTML =
      "      <b>Lookup will be cleared (will be set to null)</b>";

    container.parentElement.appendChild(makeNullDiv);

    const lookupEditLinks = document.getElementsByClassName("lookupEditLinks");
    const lookupEditLinkDiv = [...lookupEditLinks].find(
      (l) => l.dataset.fieldname === fieldName
    );
    ...
    lazyLookupInitFunctions[fieldName] = async () =>
      initLookupMetadata(targetToCache);

    setLookupInputQueryHandlers(
      queryInput,
      selectTable,
      selectFilterField,
      resultsSelect
  ...
  async function handleTableSelectv2(selectTable, selectFilterField) {
    ...
    const { name, key, plural } = await retrievePrimaryNameAndKeyAndPluralName(
      logicalName
    );
  ...
  async function setLookupInputQueryHandlers(
    input,
    selectTable,
    selectFilterField,
    resultsSelect
    ...
    input.oninput = async () => {
      ...
      const primaryNameAndKeyAndPlural =
        await retrievePrimaryNameAndKeyAndPluralName(selectTable.value);
      ...
      const results = await doLookupInputQuery(
        pluralName,
        fieldToQuery,
        primaryfieldname,
        primaryKeyName,
  ...
  async function editRecord(logicalName, pluralName, id) {
    ...
    const inputContainers = document
      .getElementsByClassName("mainPanel")[0]
      .getElementsByClassName("inputContainer");
    const inputContainersArray = [...inputContainers];

    for (let attribute of attributesMetadata) {
      let container = inputContainersArray.find(
        (c) => c.dataset.fieldname === attribute.LogicalName
      );
      ...
      if (attributeType === "String") {
        ...
        const fieldOptionSetMetadata = optionSetMetadata.find(
          (osv) => osv.LogicalName === attribute.LogicalName
        );
        if (fieldOptionSetMetadata) {
          const fieldOptionset =
            fieldOptionSetMetadata.GlobalOptionSet ||
            fieldOptionSetMetadata.OptionSet;
        ...
        const fieldOptionSetMetadata = booleanMetadata.find(
          (osv) => osv.LogicalName === attribute.LogicalName
        );
        if (fieldOptionSetMetadata) {
          const fieldOptionset =
          ...
          createBooleanInput(
            container,
            fieldOptionset.FalseOption,
            fieldOptionset.TrueOption
          );
        }
    ...
    const notEnabledContainers = document
      .getElementsByClassName("mainPanel")[0]
      .getElementsByClassName("containerNotEnabled");
    ...
    const editMenuDiv = document
      .getElementsByClassName("mainPanel")[0]
      .getElementsByClassName("editMenuDiv")[0];
    editMenuDiv.style.display = "inline";

    const submitLink = document
      .getElementsByClassName("mainPanel")[0]
      .getElementsByClassName("submitLink")[0];
  ...
  async function submitEdit(pluralName, id) {
    const previewChangesBeforeSaving = document
      .getElementsByClassName("mainPanel")[0]
      .getElementsByClassName("previewChangesBeforeSavingBox")[0].checked;

    const changedFields = {};
    const enabledFields = document
      .getElementsByClassName("mainPanel")[0]
      .getElementsByClassName("enabledInputField");
      ...
      if (dataType === "string" || dataType === "memo") {
        ...
        if (
          ...
          if (inputValue.includes(",")) {
            alert(
              `${fieldName} is a column of type '${dataType}' and contains a comma (,). Use a dot (.) as the separator.`
            );
            return;
          }
          ...
          if (isNaN(value)) {
            alert(
              `${fieldName} is a column of type '${dataType}'. The value ${inputValue} is not compatible.`
            );
            return;
          }
        ...
        const tablesSelects =
          document.getElementsByClassName("lookupSelectTable");
        const tableSelect = [...tablesSelects].find(
        ...
        if (tableSelectValue == null || tableSelectValue === "") {
          alert(
            "Error for lookup field: " +
              fieldName +
              ". No table was selected. This should not be possible."
          );
          return;
        }
        ...
        if (input.dataset.editmode === "update") {
          if (inputValue == null || inputValue === "") {
            alert(
              "Error for lookup field: " +
                fieldName +
                '. The field was marked for update but it is empty. If you do not want to edit this field, hit "undo changes".'
            );
            return;
          }
      ...
      if (value !== originalValue && !(value === "" && originalValue == null)) {
        if (dataType === "memo") {
          ...
          if (!!previewChangesBeforeSaving) {
            ...
            changedFields[lookupFieldName + "____lookupOverrideOriginalValue"] =
              originalValue;
          }
    ...
    const impersonateAnotherUser = document.getElementsByClassName(
      "impersonateAnotherUserCheckbox"
    )[0].checked;
    const impersonateAnotherUserField = document.getElementsByClassName(
      "impersonateAnotherUserSelect"
    )[0].value;
    const impersonateAnotherUserInput = document.getElementsByClassName(
      "impersonateAnotherUserInput"
    ...
    if (!!impersonateAnotherUser) {
      if (
        impersonateAnotherUserInput == null ||
        impersonateAnotherUserInput === ""
      ) {
        alert(
          "User impersonation was checked, but " +
            impersonateAnotherUserField +
            " is empty"
        );
        return;
      }
      ...
      if (impersonateAnotherUserInput?.length !== 36) {
        alert(
          "User impersonation input error: " +
            impersonateAnotherUserInput +
            " is not a valid guid."
        );
        return;
      ...
      if (impersonateAnotherUserField == "systemuserid") {
        ...
        impersonateAnotherUserField == "azureactivedirectoryobjectid"
      ) {
        impersonateHeader["CallerObjectId"] = impersonateAnotherUserInput;
      } else {
        alert(
          "This should not happen. Wrong value in impersonateAnotherUserSelect: " +
            impersonateAnotherUserField
        );
        return;
      }
  ...
  async function commitSave(pluralName, id, changedFields, impersonateHeader) {
    ...
    const bypassCustomPluginExecution = document.getElementsByClassName(
      "bypassPluginExecutionBox"
    )[0].checked;
  ...
  function isAnnotation(key) {
    return (
      stringContains(key, formattedValueType) ||
      stringContains(key, navigationPropertyType) ||
      stringContains(key, lookupType)
    );
  }

  async function prettifyWebApi(
    jsonObj,
    htmlElement,
    pluralName,
    generateEditLink
    ...
    if (isMultiple) {
      ...
      for (const key in jsonObj[valueKeyWithCount]) {
        jsonObj[valueKeyWithCount][key] = await enrichObjectWithHtml(
          jsonObj[valueKeyWithCount][key],
          result.logicalName,
          pluralName,
          result.primaryIdAttribute,
          false,
          false,
          2
        );
      ...
      jsonObj = await enrichObjectWithHtml(
        jsonObj,
        result.logicalName,
        pluralName,
        result.primaryIdAttribute,
  ...
  async function makeItPretty() {
    ...
    window.currentEntityPluralName = window.location.pathname
      .split("/")
      .pop()
      .split("(")
      .shift();
    ...
    await prettifyWebApi(
      response,
      document.body,
      window.currentEntityPluralName,
      true
    );
  }
